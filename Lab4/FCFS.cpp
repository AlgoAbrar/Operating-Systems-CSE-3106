...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        cout << "Process " << processid[i] << ": Arrival Time = " << arrival[i] << ", Brust Time = " << brust[i] << ", Completion Time = " << completion[i] << ", Turnaround Time = " << turnaround[i] << ", Waiting Time = " << waiting[i] << endl;
    }
}